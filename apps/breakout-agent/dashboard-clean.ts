...
async function dashboard() {
  ...
  console.log("\n" + "=".repeat(90));
  console.log(
    "SIGNAL FORGE DASHBOARD - TYPE 1 (BREAKOUTS) + TYPE 2 (SETUPS)".padStart(
      60,
    ),
  );
  ...
  for (const sig of breakouts.slice(0, 10)) {
    ...
    console.log(
      `${sig.asset.padEnd(5)} | ${conf}%  | ${pine}  | ${price} | ${res} | ${sup}`,
    );
  }
  ...
  console.log("-".repeat(90));
  console.log(
    "ASSET | TYPE    | DISTANCE | MA20    | CONFIDENCE | CONSOLIDATION QUALITY",
  );
  console.log("-".repeat(90));
  ...
  for (const sig of setups) {
    ...
    const conf = (sig.confidence * 100).toFixed(0).padStart(3);
    const range =
      m.setupConsolidationRangePercent?.toFixed(1).padStart(4) || "   0";
    const vol =
      m.setupConsolidationVolumePercent?.toFixed(0).padStart(2) || " 0";
    const quality = `${range}% range / ${vol}% vol`;
    console.log(
      `${sig.asset.padEnd(5)} | ${type} | ${dist}% | ${ma20} | ${conf}%       | ${quality}`,
    );
  }

  console.log("\n" + "=".repeat(90));
  console.log(
    `SUMMARY: ${breakouts.length} breakouts scanned | ${setups.length} setups detected | S&P 500 suite active`,
  );
  console.log("=".repeat(90) + "\n");
