...
async function monitorSignals() {
  ...
  while (true) {
    ...
    if (breakoutCount > lastBreakoutCount || setupCount > lastSetupCount) {
      console.clear();
      console.log(
        `[${new Date().toLocaleTimeString()}] Signal Forge - Live Dashboard`,
      );
      console.log(`${"=".repeat(70)}`);
      console.log(
        `Type 1 (Breakouts):        ${breakoutCount.toString().padStart(5)} total | ${highConfidenceBreakouts.toString().padStart(3)} high conf (≥90%)`,
      );
      console.log(
        `Type 2 (Setups):           ${setupCount.toString().padStart(5)} total | ${highConfidenceSetups.toString().padStart(3)} high conf (≥85%)`,
      );
      console.log(`${"=".repeat(70)}`);
