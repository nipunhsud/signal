...
async function dashboard() {
  ...
  console.log('-'.repeat(80));
  console.log('ASSET | CONF | PINE | BARS | PRICE   | RESISTANCE | SUPPORT  | TIME');
  console.log('-'.repeat(80));
  breakouts.slice(0, 5).forEach((sig) => {
    ...
    console.log(`${sig.asset.padEnd(5)} | ${conf}% | ${pine}  | ${bars}   | ${price} | ${res} | ${sup} | ${time}`);
  });
  ...
  console.log('-'.repeat(80));
  console.log('ASSET | TYPE    | DIST  | MA20   | CONF  | CONSOLIDATION | TIME');
  console.log('-'.repeat(80));
  setups.forEach((sig) => {
    ...
    const range = meta.setupConsolidationRangePercent?.toFixed(1).padStart(3) || '  0';
    const vol = meta.setupConsolidationVolumePercent?.toFixed(0).padStart(2) || ' 0';
    const time = new Date(sig.createdAt).toLocaleTimeString().slice(0, 8);
    console.log(`${sig.asset.padEnd(5)} | ${type} | ${dist}% | ${ma20} | ${conf}% | ${range}% / ${vol}% | ${time}`);
  });
